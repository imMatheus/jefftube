...
export function VideoCard({
  ...
  return (
    <div className={cn('group flex-shrink-0', sizeClasses[size])}>
      ...
      <div className="flex gap-2">
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-1">
            <h3 className="text-sm font-medium text-(--color-text-primary) line-clamp-2 leading-5">{title}</h3>
            <button className="opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 p-1 -mr-1 hover:bg-(--color-bg-hover) rounded-full">
              <MoreVertIcon />
            </button>
          </div>
          {showChannel && channel && (
            <div className="flex items-center gap-1 mt-1">
              <span className="text-xs text-(--color-text-secondary) hover:text-(--color-text-primary) cursor-pointer">{channel}</span>
